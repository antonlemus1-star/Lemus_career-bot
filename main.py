...
async def process_message(msg: dict):
    ...
    if is_admin and text.startswith("/set_balance"):
        ...
        if len(parts) == 3:
            try:
                ...
                await send_telegram(chat_id, "⚠️ Формат: `/set_balance 123456789 100`")
        return
    ...
    if text.startswith("/start"):
        await send_telegram(chat_id, "👋 Привет! Твой карьерный агент готов к работе.\n\n🎁 Вам начислено *30 приветственных запросов*!\nПриглашайте друзей через кнопку «👥 Пригласить друга» и получайте по 30 запросов за каждого участника.", get_keyboard(is_admin))
        ...
        ref_text = (
            ...
            "Поделитесь своей ссылкой с друзьями. Как только друг запустит бота, **и вы, и он получите по 30 бесплатных запросов**!\n\n"
            f"🔗 Ваша ссылка:\n`{ref_link}`"
        )
        ...
        help_text = (
            ...
            "📁 *Мои резюме / Загрузить резюме* — загружайте файлы PDF, DOCX, RTF или TXT, переключайте активные версии.\n"
            "🔍 *Поиск вакансий* — ИИ подбирает подходящие позиции с hh.ru под активное резюме (1 запрос).\n"
        ...
        await send_telegram(chat_id, "🛠 *Выберите резюме* для адаптации под вакансию:", kb)
        ...
        balance_text = (
            ...
            "💳 *Способы пополнения:*\n"
            "1. **Реферальная программа:** Нажмите «👥 Пригласить друга» (+30 запросов за друга).\n"
            "2. **Telegram Stars (⭐):** Оплата внутри Telegram.\n"
            "3. **Перевод с карты / СБП:** Свяжитесь с администратором: "
            f"{f'@{ADMIN_ID}' if ADMIN_ID else 'администратором сервиса'}, указав ID (`{chat_id}`)."
        )
