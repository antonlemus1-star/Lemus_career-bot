...
async def cmd_help(message: types.Message):
    help_text = (
        ...
        "🔍 **Поиск вакансий** — автоматический подбор 5 свежих вакансий с HH.ru за 10 дней.\n"
        ...
        "📌 **Трекер откликов** — канбан-доска для управления статусами собеседований.\n"
...
async def process_cv_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    if current_state == CareerState.choosing_cv_for_search.state:
        ...
        if await check_and_deduct(callback.from_user.id, callback.message):
            ...
            prompt = (f"Сформируй 1-3 ключевых слова для строки поиска HH.ru. Верни ТОЛЬКО слова через пробел. "
                      f"Исключи нежелательные названия: {dislikes_str}.\n\nРЕЗЮМЕ:\n{cv_text[:1000]}")
        ...
        if await check_and_deduct(callback.from_user.id, callback.message):
            prompt = f"Проведи глубокий аудит резюме, подсвети клише и точки роста:\n\n{cv_text}"
            res = ai_client.models.generate_content(model=MODEL_NAME, contents=prompt)
...
async def main():
    ...
    print("Бот успешно запущен со всем функционалом!")
