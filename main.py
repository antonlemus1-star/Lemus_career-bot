...
def register_user(user_id: int, username: str, referrer_id: int = None) -> bool:
  ...
  if referrer_id:
    cur.execute(
        "UPDATE users SET balance = balance + 30 WHERE user_id=?",
        (referrer_id,),
    )
    conn.commit()
...
def add_resume(user_id: int, name: str, text: str):
  ...
  cur.execute(
      "INSERT INTO resumes (user_id, name, text, active) VALUES (?,?,?,1)",
...
def get_active_resume(user_id: int) -> str:
  cur.execute(
      "SELECT text FROM resumes WHERE user_id=? AND active=1 ORDER BY id DESC"
      " LIMIT 1",
      (user_id,),
  )
  row = cur.fetchone()
  if not row:
    cur.execute(
        "SELECT text FROM resumes WHERE user_id=? ORDER BY id DESC LIMIT 1",
        (user_id,),
    )
    row = cur.fetchone()
...
def get_resume_by_id(user_id: int, resume_id: int) -> str:
  cur.execute(
      "SELECT text FROM resumes WHERE id=? AND user_id=?", (resume_id, user_id)
  )
  row = cur.fetchone()
  return row[0] if row else ""
...
def hide_vacancy(user_id: int, vacancy_id: str):
  cur.execute(
      "INSERT OR IGNORE INTO hidden_vacancies (user_id, vacancy_id) VALUES (?,"
      " ?)",
      (user_id, vacancy_id),
  )
  conn.commit()
...
def is_vacancy_hidden(user_id: int, vacancy_id: str) -> bool:
  cur.execute(
      "SELECT 1 FROM hidden_vacancies WHERE user_id=? AND vacancy_id=?",
      (user_id, vacancy_id),
  )
  ...
  if GROQ_KEY:
    try:
      ...
      return _openai_compat(
          prompt, "https://api.groq.com/openai/v1", GROQ_KEY, GROQ_MODEL
      )
    except Exception as e:
...
async def send_single_message(
    chat_id, text: str, reply_markup=None, parse_mode="Markdown"
):
...
async def send_telegram(
  ...
  """Умная отправка: если текст длиннее 4000 символов, делит на части."""
  if not text:
    return
  if len(text) <= 4000:
...
async def send_document_bytes(
    chat_id, file_bytes: bytes, filename: str, caption: str = ""
):
...
async def hh_api_search(query: str):
  try:
    async with HTTP.get(
        ...
        headers={
            "User-Agent": "LemusCareerBot/1.0 (career-bot)",
            "HH-User-Agent": "LemusCareerBot/1.0",
        },
    ) as resp:
    ...
    for i in data.get("items", []):
      items.append({
          ...
          "url": (
              i.get("alternate_url") or f"https://hh.ru/vacancy/{i.get('id')}"
          ),
      })
...
async def handle_search(chat_id: int, is_admin: bool):
  if not spend_balance(chat_id, cost=1):
    await send_telegram(
        chat_id,
        "⚠️ У вас закончились запросы! Пополните баланс через меню «💎 Оплата и"
        " Баланс» или пригласите друзей.",
  ...
  prompt = (
      "Проанализируй текст резюме и напиши только короткое название должности"
      " для поиска на hh.ru (2-4 слова, без кавычек). Пример ответа:"
      f" Руководитель проектов\n\nРезюме: {resume[:2000]}"
  )
  ...
  if not query or len(query.strip()) > 50:
    await send_telegram(
        chat_id,
        "⚠️ Не удалось автоматически подобрать запрос. Напиши, какую должность"
        " ты ищешь?",
        get_keyboard(is_admin),
    )
    return
  ...
  if not items:
    await send_telegram(
        chat_id,
        f"⚠️ Не удалось найти вакансии по запросу «{query}». Попробуй написать"
        " должность вручную.",
        get_keyboard(is_admin),
    )
    return
  ...
  filtered_items = [
      v for v in items if not is_vacancy_hidden(chat_id, str(v["id"]))
  ]
  if not filtered_items:
    await send_telegram(
        chat_id,
        f"⚠️ Все найденные вакансии по запросу «{query}» находятся в вашем"
        " черном списке.",
        get_keyboard(is_admin),
    )
    return
  ...
  await send_telegram(
      chat_id,
      f"🔥 Нашел позиций по запросу «{query}» (доступно:"
...
async def run_ai_generation(chat_id: int, vac_info: dict):
  await send_telegram(
      chat_id,
      f"✍️ Готовлю сопроводительное письмо для *{vac_info['employer']}* на"
      f" позицию «{vac_info['title']}»...",
  )
  resume = get_active_resume(chat_id) or "Опыт не указан."
  letter = await asyncio.to_thread(
      ai_generate,
      "Напиши профессиональное сопроводительное письмо на позицию"
      f" '{vac_info['title']}' в '{vac_info['employer']}' на основе"
      f" резюме:\n\n{resume}",
  )
  if not letter:
...
async def run_vacancy_match(chat_id: int, vac_info: dict):
  await send_telegram(
      chat_id,
      "📊 Анализирую соответствие вашего резюме вакансии"
      f" *{vac_info['title']}* в компании *{vac_info['employer']}*...",
  )
  resume = get_active_resume(chat_id) or "Резюме не найдено."
  prompt = (
      "Проанализируй, насколько резюме кандидата подходит под вакансию"
      f" '{vac_info['title']}' в компанию '{vac_info['employer']}'. Дай оценку"
      " соответствия в процентах, перечисли сильные стороны кандидата для этой"
      " роли, а также укажи ключевые пробелы (что нужно исправить или добавить"
      f" в резюме):\n\nРезюме:\n{resume}"
  )
  analysis = await asyncio.to_thread(ai_generate, prompt)
  if not analysis:
    await send_telegram(
        chat_id, "⚠️ ИИ временно недоступен, не удалось провести анализ."
    )
    return
  await send_telegram(
      chat_id, f"📊 *Анализ соответствия вакансии:*\n\n{analysis}"
  )


async def run_resume_adaptation(chat_id: int, resume_id: int, vacancy_text: str):
  await send_telegram(
      chat_id, "🛠 Адаптирую выбранное резюме под указанную вакансию..."
  )
  ...
  if not resume_text:
    await send_telegram(
        chat_id, "⚠️ Резюме не найдено! Загрузите файл резюме."
    )
    return

  prompt = (
      "Перепиши и оптимизируй текст резюме кандидата так, чтобы оно идеально"
      " подходило под требования указанной вакансии (выдели ключевые навыки,"
      " перефразируй достижения с учетом требований работодателя, сохрани"
      " правдивость опыта):\n\n--- ТРЕБОВАНИЯ ВАКАНСИИ"
      f" ---\n{vacancy_text[:3000]}\n\n--- ИСХОДНОЕ РЕЗЮМЕ"
      f" ---\n{resume_text[:6000]}"
  )
  ...
  if not adapted_text:
    await send_telegram(
        chat_id, "⚠️ ИИ временно недоступен, не удалось адаптировать резюме."
    )
    return
...
async def run_resume_audit(chat_id: int):
  ...
  await send_telegram(
      chat_id,
      "📋 *Провожу глубокий аудит резюме...*\n\n1. Анализирую красные флаги и"
  ...
  # Запрос 1: Подробный аудит в чат (с требованием дописать до конца)
  audit_prompt = (
      "Ты жесткий IT-рекрутер и C-level менеджер. Сделай структурированный и"
      " профессиональный аудит резюме.\nУкажи:\n1. Главные Deal-breakers"
      " (критические ошибки позиционирования).\n2. Топ-4 красных флага.\n3."
      " Краткий разбор опыта по компаниям (плюсы и минусы).\n4. Четкий план"
      " действий по исправлению.\n\nОбязательно доведи разбор до логического"
      f" финала, не обрывая текст на середине!\n\nРезюме:\n{resume[:8000]}"
  )
  audit_text = await asyncio.to_thread(ai_generate, audit_prompt)

  # Запрос 2: Чистовое резюме для файла DOCX
  rewrite_prompt = (
      "Перепиши это резюме в идеальный профессиональный формат с сохранением"
      " структуры блоков (Контакты, О себе, Навыки, Опыт работы, Образование)."
      " Усиль формулировки цифрами, достижениями и бизнес-результатами. Выдай"
      " ТОЛЬКО текст резюме без лишних вводных слов и"
      f" комментариев:\n\n{resume[:8000]}"
  )
  ...
  if not audit_text or not improved_resume_text:
    await send_telegram(
        chat_id, "⚠️ ИИ временно недоступен, не удалось завершить аудит."
    )
    return

  # Отправляем полный разбор в чат (если длинный — автоматически разобьется)
  await send_telegram(chat_id, f"📋 *Результаты аудита резюме:*\n\n{audit_text}")

  # Формируем и отправляем Word-документ
  doc = Document()
  ...
  await send_document_bytes(
      ...
      caption=(
          "✅ *Ваше улучшенное готовое резюме*\nСтруктура сохранена, формулировки"
          " усилены."
      ),
  )
...
async def handle_ai(chat_id: int, is_admin: bool, prompt: str):
  ...
  if not answer:
    await send_telegram(
        chat_id, "⚠️ ИИ недоступен, попробуй позже.", get_keyboard(is_admin)
    )
  else:
...
async def handle_document(chat_id: int, document: dict, is_admin: bool):
  ...
  try:
    ...
    if not file_path:
      await send_telegram(
          chat_id, "⚠️ Не смог скачать файл.", get_keyboard(is_admin)
      )
      return
    async with HTTP.get(
    ...
    await send_telegram(
        chat_id, "⚠️ Ошибка скачивания файла.", get_keyboard(is_admin)
    )
    return
  ...
  if not text_content or not text_content.strip():
    await send_telegram(
        chat_id,
        "⚠️ Не извлёк текст. Поддерживаю PDF, DOCX, RTF, TXT.",
        get_keyboard(is_admin),
    )
    return
  ...
  await send_telegram(
      chat_id,
      f"✅ Резюме «{file_name}» успешно распознано и назначено активным!",
      get_keyboard(is_admin),
  )
...
async def activate_resume(chat_id: int, rid: str):
  ...
  cur.execute(
      "UPDATE resumes SET active=1 WHERE id=? AND user_id=?", (rid, chat_id)
  )
  conn.commit()
  await send_telegram(
      chat_id,
      "✅ Резюме сделано активным.",
...
async def process_message(msg: dict):
  ...
  if is_admin and text.startswith("/add_credits"):
    ...
    if len(parts) == 3:
      try:
        ...
        await send_telegram(
            chat_id,
            f"✅ Пользователю `{target_id}` добавлено {amount} запросов. Баланс:"
            f" `{new_bal}`",
        )
      except ValueError:
        await send_telegram(
            chat_id, "⚠️ Формат: `/add_credits 123456789 50`"
        )
    return
  ...
  if is_admin and text.startswith("/set_balance"):
    ...
    if len(parts) == 3:
      try:
        ...
        await send_telegram(
            chat_id,
            f"✅ Пользователю `{target_id}` установлен баланс: `{amount}`"
            " запросов.",
        )
      except ValueError:
        await send_telegram(
            chat_id, "⚠️ Формат: `/set_balance 123456789 100`"
        )
    return
  ...
  if user_states.get(chat_id) == "waiting_for_adaptation_vacancy":
    ...
    if not spend_balance(chat_id, cost=1):
      await send_telegram(
          chat_id,
          "⚠️ У вас закончились запросы! Пополните баланс через меню «💎 Оплата"
          " и Баланс» или пригласите друзей.",
          get_keyboard(is_admin),
      )
      return
  ...
  if text.startswith("/start"):
    await send_telegram(
        ...
        " *30 приветственных запросов*!\nПриглашайте друзей через кнопку «👥"
        " Пригласить друга» и получайте по 30 запросов за каждого участника.",
        get_keyboard(is_admin),
    )
    ...
    ref_text = (
        "👥 *Реферальная система «Приведи друга»*\n\nПоделитесь своей ссылкой с"
        " друзьями. Как только друг запустит бота, **и вы, и он получите по 30"
        f" бесплатных запросов**!\n\n🔗 Ваша ссылка:\n`{ref_link}`"
    )
    ...
    help_text = (
        ...
        "📁 *Мои резюме / Загрузить резюме* — загружайте файлы PDF, DOCX, RTF"
        " или TXT, переключайте активные версии.\n"
        "🔍 *Поиск вакансий* — ИИ подбирает подходящие позиции с hh.ru под"
        " активное резюме (1 запрос).\n"
        "  • *Сопроводительное* — готовое письмо под вакансию.\n"
        "  • *Соответствие* — детальный разбор шансов и рекомендации.\n"
        "  • *Мусор* — скрывает вакансию из выдачи.\n"
        "🛠 *Адаптация резюме* — переработка выбранного резюме под текст вакансии"
        " (1 запрос).\n"
        "📊 *Анализ навыков (Skill Gap)* — выявление пробелов в навыках (1"
        " запрос).\n"
        "📋 *Аудит резюме* — жесткая критика в чате и формирование готового"
        " Word-файла (.docx) с улучшениями (1 запрос).\n"
        "🎤 *Тренажер собеседований* — симуляция каверзных вопросов (1"
        " запрос).\n"
        "📌 *Трекер откликов* — учет отправленных заявок.\n"
        "💎 *Оплата и Баланс* — проверка остатка запросов и способы"
        " пополнения."
    )
    ...
    if not rows:
      await send_telegram(
          chat_id, "⚠️ Сначала загрузите резюме!", get_keyboard(is_admin)
      )
      return
    kb = {
        "inline_keyboard": [[{
    ...
    await send_telegram(
        chat_id, "🛠 *Выберите резюме* для адаптации под вакансию:", kb
    )
    ...
    balance_text = (
        ...
        " Нажмите «👥 Пригласить друга» (+30 запросов за друга).\n2. **Telegram"
        " Stars (⭐):** Оплата внутри Telegram.\n3. **Перевод с карты / СБП:**"
        " Свяжитесь с администратором:"
        f" {f'@{ADMIN_ID}' if ADMIN_ID else 'администратором сервиса'}, указав"
        f" ID (`{chat_id}`)."
    )
    ...
    if not rows:
      await send_telegram(
          chat_id, "⚠️ Нет загруженных резюме.", get_keyboard(is_admin)
      )
    else:
      kb = {
          "inline_keyboard": [[{
              "text": f"{'✅' if r['active'] else '📄'} {r['name']}",
              "callback_data": f"act_{r['id']}",
          }] for r in rows]
      }
      await send_telegram(
          chat_id, "📁 *Твои резюме:* (нажми, чтобы сделать активным)", kb
      )

  elif text == "📥 Загрузить резюме":
    await send_telegram(
        chat_id,
        "📄 Отправь файл резюме (PDF, DOCX, RTF или TXT) в чат.",
        get_keyboard(is_admin),
    )
    ...
    if not get_active_resume(chat_id):
      await send_telegram(
          chat_id, "⚠️ Сначала загрузи резюме!", get_keyboard(is_admin)
      )
    else:
    ...
    await send_telegram(
        chat_id, "📌 Твои отклики пока пусты.", get_keyboard(is_admin)
    )
    ...
    if "Анализ навыков" in text:
      prompt = (
          "Проведи Skill Gap анализ для руководителя проектов:\n\n"
          f"{resume[:8000]}"
      )
    elif "Тренажер" in text:
      prompt = (
          "Ты жесткий интервьюер. Задай мне первый каверзный вопрос для"
          " руководителя проектов."
      )
    else:
      prompt = (
          "Ты карьерный консультант. Контекст резюме"
    ...
    if chat_id:
      if data_str.startswith("gen_"):
        if not spend_balance(chat_id, cost=1):
          await send_telegram(
              chat_id,
              "⚠️ У вас закончились запросы! Пополните баланс через меню «💎"
              " Оплата и Баланс» или пригласите друзей.",
          )
          return
        v = temp_vacancies.get(
            data_str[4:], {"title": "Вакансия", "employer": "Компания"}
        )
        ...
        if not spend_balance(chat_id, cost=1):
          ...
          return
        v = temp_vacancies.get(
            data_str[6:], {"title": "Вакансия", "employer": "Компания"}
        )
        ...
        bg(
            http_edit_message_text(
                chat_id,
                message_id,
                "✅ Резюме выбрано!\n\nТеперь **скопируйте и вставьте текст"
                " вакансии** в ответное сообщение.",
            )
        )
        ...
        bg(
            http_edit_message_text(
                chat_id,
                message_id,
                "🗑 Вакансия помечена как мусор и больше не будет показываться.",
            )
        )
    return web.Response(text="OK")
...
def log_startup():
  ...
  if providers:
    log.info(
        "🟢 ИИ-провайдеры активны: %s (%d из 2)",
        ", ".join(providers),
        len(providers),
    )
  else:
